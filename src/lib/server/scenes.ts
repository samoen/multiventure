...
export type SceneKey = 'forest' | 'castle' | 'throne' | 'forestPassage' | 'goblinCamp' | 'tunnelChamber' | 'postBattleThrone';
export type Scene = {
...
export const scenes : Record<SceneKey,Scene> = {
			...
			},
			{
				...
				generate: (actor: Player) => {
					...
					return {
						...
						performAction: () => {
							actor.transitionSceneText="After what feels like hours scrambling in the fetid soil and dodging the bites of the foul crawling creatures that call the forest home, you stumble upon an entrace."
		...
		sceneActions: [
			{
				...
				generate(actor) {
					return {
						...
						performAction(){
							actor.transitionSceneText = 'Despite your rising panic at the mere thought of entering that hellish maze of rotting plant matter and creatures beyond imagination, you push your way back into the depths.'
			...
			},
			{
				...
				generate(actor) {
					return {
						...
						performAction(){
							actor.transitionSceneText = 'At the end of the entrace hall to this eldritch structure, you notice a great door. It seems to stretch up into infinity, and you see no handle. As you approach, it seems to crack a part, revealing a dazzling light. You step inside.'
				...
				generate(actor){
					...
					return{
						...
						performAction() {
							...
							actor.transitionSceneText = "You wend your way down a neverending series of corridors and pathways that seem to go on for an enternity. It becomes narrower and narrower, and the heat becomes almost unbearable. The path suddenly opens into a great chamber."
						},
	...
	forestPassage: {
		mainSceneText: " It's so dark that you can hardly make out an exit. Feeling around, your hand brush against the walls. They feel warm. As if they were alive.",
		onEnterScene(user){
			if (!user.flags.has('gotFreeStarterWeapon')) {
				user.duringSceneText = "The walls begin to contort and bend, and from the the strangely organic surface a face begins to form. Its lip start to move. 'You have made it further than most' It creaks. You make as if to run but from the walls come arms that bind you in place. 'Please. I mean you no harm' The walls murmur. In your mind you see the image of a golden sword, and beside it a bow of sturdy but flexible oak. You realise you are being given a choice.";
			}
		},
		sceneActions: [
			{
					...
					return {
						...
						performAction() {
							if(actor.flags.has('killedGoblins')){
								actor.transitionSceneText = `You push through to a familiar camp. It's as you left it.`
							}else{
								actor.transitionSceneText = "Urged on by by your own fear and by some unknown inspiration, you fumble your way through the darkness towards the light. You are blinded as you step through and are greeted with the site of a ramshackle encampment"
							}
	...
	goblinCamp:{
		mainSceneText:"There is a foul stench in the air. Goblins. The telltale signs of the disgusting beasts are everywhere. Various animal carcasses litter the area, and their homes, barely more than logs with tattered cloth strung between, are placed without method around the clearing.",
		onEnterScene(user) {
			if(!(activeEnemies.some(e=>e.name=='Gorlak') || activeEnemies.some(e=>e.name=='Murk')) && !user.flags.has('killedGoblins')){
				user.duringSceneText = `Suddendly, A pair of goblins rush out of a tent.. "Hey Gorlak, looks like lunch!" "Right you are Murk. Let's eat!"`
				activeEnemies.push({
					name:'Gorlak',
				...
				generate(actor) {
					...
					return {
						...
						performAction() {
							...
							actor.duringSceneText = 'you chose to place it!! fight the stranger'
							activeEnemies.push({
								name:'stranger',
								currentHealth:40,
			...
			},
			{
				...
				generate(actor) {
					...
					return {
						...
						performAction() {
							actor.duringSceneText = 'you killed stranger'
							actor.currentScene = 'postBattleThrone'
							actor.transitionSceneText = "you go from fighting stranger to throne"
						},
					}
				},
			},
	...
	postBattleThrone:{
		mainSceneText:"your in a throne thats different because yuou fought someone down there",
		sceneActions:[
