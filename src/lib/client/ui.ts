...
export const convoStateForEachVAS: Writable<
...
export const vasesToShow = derived(
	[visualActionSources, convoStateForEachVAS],
	([$visualActionSources, $convoStateForEachVAS]) => {
...
export function resetSceneConvos(sceneId: SceneDataId) {
	const vasesToReset = get(visualActionSources).filter((v) => v.scene == sceneId);
...
export const currentAnimation = derived(
	[currentAnimationIndex, currentAnimationsWithData],
	([$currentAnimationIndex, $currentAnimationsWithData]) => {
...
export const typedInventory = derived(
	[lastMsgFromServer, waitingForMyAnimation, clientState],
	([$lastMsgFromServer, $waitingForMyAnimation, $clientState]) => {
...
export const selectedDetail: Readable<DetailWindow | undefined> = derived(
	[lastUnitClicked, allVisualUnitProps, vasesToShow, convoStateForEachVAS],
	([$lastUnitClicked, $allVisualUnitProps, $vases, $convoStateForEachVAS]) => {
		...
		const vasAt = $vases.find((v) => v.id == $lastUnitClicked);
...
export const selectedVisualActionSourceState = derived(
	...
	([
		...
		const csForE = $convoStateForEachVAS.get($selectedDetail.entity.scene);
		if (!csForE) return undefined;
		const state = csForE.get($selectedDetail.entity.id);
		if (!state) {
			return undefined;
		}
		return state;
...
export const selectedVasResponsesToShow = derived(
	[selectedVisualActionSourceState, selectedDetail],
	([$selectedVisualActionSourceState, $selectedDetail]) => {
		if (!$selectedDetail || !$selectedVisualActionSourceState || $selectedDetail.kind != 'vas')
			return [];
...
export const selectedVasActionsToShow = derived(
	[selectedVisualActionSourceState, selectedDetail],
	([$selectedVisualActionSourceState, $selectedDetail]) => {
		if (!$selectedDetail || !$selectedVisualActionSourceState || $selectedDetail.kind != 'vas')
			return [];
...
export const centerFieldTarget = derived(
	...
	([$currentAnimation, $subAnimationStage]) => {
		...
		if ($currentAnimation.behavior.kind == 'center' && $subAnimationStage == 'fire') {
...
export async function nextAnimationIndex(start: boolean, someoneDied: boolean) {
	...
	const curAnimations = get(currentAnimationsWithData);
	const latest = get(lastMsgFromServer);
	if (!latest) {
...
function checkAnimationValid(ba: BattleAnimation): boolean {
	const enemiesToCheck = get(enemies);
	const alliesToCheck = get(allies);
	const vasesToCheck = get(vasesToShow);
...
export async function worldReceived(sMsg: MessageFromServer) {
	const prevMsg = structuredClone(get(lastMsgFromServer));
